...
// Load collaborative recommendations (exported by Python precompute helper)
const collabPath = path.join(__dirname, 'data', 'collab_recs.json');
if (fs.existsSync(collabPath)) {
  try {
    app.locals.collabMap = JSON.parse(fs.readFileSync(collabPath, 'utf8'));
...
// mount under /api/recommendations (new) and /recommendations (legacy FastAPI paths)
app.use('/api/recommendations', recommendations);
app.use('/recommendations', recommendations);

app.get('/', (req, res) => {
  res.json({ message: 'Book Recommendation MERN server', status: 'ok' });
});
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({ status: 'healthy', books: booksCount, ratings: ratingsCount, collaborative_ready: !!app.locals.collabMap });
  } catch (err) {
